...
app.get('/valid', verifyUser, (req, res) => {
  ...
  if(key === valkey){
    ...
    app.delete('/vald/id/:id', async (req, res) => {
      try {
...
app.post('/gif/data/gifno1', async (req, res) => {
  const { gifno1 } = req.body;
  
  // Check if the no exists in the database
  const user = await GifWonDataModel.findOne({ gifno1 });
...
app.post('/gif/won/data', async (req, res) => {
  try {
    ...
    res.status(205).json({ Status : "BAD" });
  }
});
...
app.post('/won/n1/data', async (req, res) => {
  const { no } = req.body;
  // Check if the no exists in the database
...
app.delete('/way/md/:id', async (req, res) => {
  try {
...
app.delete('/wy1/dle/:id', async (req, res) => {
  try {
...
app.delete('/lang/dt/dl/:id', async (req, res) => {
  try {
...
app.put('/lang/dtttt/put/:email', async (req, res) => {
  const { email } = req.params;
  const { Lang } = req.body;
...
app.put('/star/totl/put/:email', async (req, res) => {
  const { email } = req.params;
  const { stars } = req.body;
...
const WonValModule = mongoose.model('WonVal', WonValSchema)

app.post('/chek/emil/exst', async (req, res) => {
  const { email } = req.body;
  
  // Check if the email exists in the database
  const user = await WonValModule.findOne({ email });
...
app.delete('/won/dl/:id', async (req, res) => {
  try {
    const item = await WonValModule.findByIdAndRemove(req.params.id);
    if (!item) {
      return res.status(201).json({ Status : "BAD" });
    }
    return res.status(200).json({ Status : "OK" });
...
app.post('/won/val', async (req, res) => {
  try {
    const Time = new Date().toLocaleString();
    const { email, rank} = req.body;
    const newPost = new WonValModule({ email, rank,Time});
    await newPost.save();
    res.status(200).json({ Status : "OK", post: newPost });
  } catch (err) {
    res.status(205).json({ error: err.message });
...
app.get('/won/val/:email', async (req, res) => {
  const { key } = req.query;
  if(key === valkey){
    try {
      const email = req.params.email;
      const user = await WonValModule.find({ email });
      if (!user) {
        return res.status(204).json({ message: 'User not found' });
      }
      res.json(user);
...
const ClamCoinModel = mongoose.model('Clamd_Coins', CoinclaimSchema)

app.get('/coin/clam/:email', async (req, res) => {
  const { key } = req.query;
  if(key === valkey){
    try {
      const email = req.params.email;
      const user = await ClamCoinModel.find({ email });
      if (!user) {
        return res.status(204).json({ message: 'User not found' });
      }
      res.json(user);
...
app.post('/coin/clm/data', async (req, res) => {
  try {
    const Time = new Date().toLocaleString();
    const { email, stars, name, valueID, valid, img} = req.body;
    const newPost = new ClamCoinModel({ email, stars, name, valueID, valid, img,Time});
    await newPost.save();
    res.status(200).json({ Status : "OK", post: newPost });
  } catch (err) {
    res.status(205).json({ error: err.message });
